const ExperienceItem = ({ company, role, description }) => (
  <div className="flex gap-4 bg-[#111714] px-4 py-3">
    <div className="text-white flex items-center justify-center rounded-lg bg-[#29382f] shrink-0 size-12">
      <svg xmlns="http://www.w3.org/2000/svg" width="24px" height="24px" fill="currentColor" viewBox="0 0 256 256">
        <path d="M216,56H176V48a24,24,0,0,0-24-24H104A24,24,0,0,0,80,48v8H40A16,16,0,0,0,24,72V200a16,16,0,0,0,16,16H216a16,16,0,0,0,16-16V72A16,16,0,0,0,216,56ZM96,48a8,8,0,0,1,8-8h48a8,8,0,0,1,8,8v8H96ZM216,72v41.61A184,184,0,0,1,128,136a184.07,184.07,0,0,1-88-22.38V72Zm0,128H40V131.64A200.19,200.19,0,0,0,128,152a200.25,200.25,0,0,0,88-20.37V200ZM104,112a8,8,0,0,1,8-8h32a8,8,0,0,1,0,16H112A8,8,0,0,1,104,112Z"></path>
      </svg>
    </div>
    <div className="flex flex-1 flex-col justify-center">
      <p className="text-white text-base font-medium leading-normal">{company}</p>
      <p className="text-[#9eb7a8] text-sm font-normal leading-normal">{description}</p>
      <p className="text-[#9eb7a8] text-sm font-normal leading-normal">{role}</p>
    </div>
  </div>
);

export default ExperienceItem;