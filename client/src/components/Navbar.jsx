...
function DesktopNavbar({ links }) {
  ...
  return (
    <div className="sticky sm:flex hidden w-[calc(100vw - .5rem)] bg-primary h-12 justify-between p-2 snap-end">
...
function MobileNavbar({ links }) {
  ...
  return (
    <>
      <div className="sm:hidden flex w-[calc(100vw - .5rem)] bg-primary h-12 justify-center p-2 snap-end">
        <button
...
const StyledLink = ({ title, link }) => {
  return (
    <Link
      className="sm:text-md lg:text-lg xl:text-xl flex flex-col justify-center px-2 py-1 min-w-[5em] text-white text-center hover:bg-background hover:text-primary transition-all"
      to={link}
    >
      <span>{title}</span>
    </Link>
